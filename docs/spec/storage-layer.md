...
| `campaign_seed` | TEXT | Master RNG seed for the entire campaign — drives terrain generation, loot tables, encounter rolls, and story archetype selection. Server-generated on campaign creation, DM can override. Digit = 0 on any position means LLM freestyles that element. |
| `heightmap_resolution` | INTEGER | Samples per 50m unit. Scales with `campaign_length`: `short` → low, `medium` → medium, `long` → high. Set once at campaign creation. |
| `world_depth` | TEXT | `standard` \| `epic` — set once at campaign creation. Standard = 7-pass world gen only. Epic = additional lore authoring and prophecy passes at campaign creation (one-time token cost). Controls Codex Lore tome availability and Epic-only features. |
| `library_access_unlocked` | BOOLEAN | Default `false`. Epic only — flipped to `true` when party completes the qualifying quest reward (`unlock_library_access`). Enables remote Codex lore access without visiting the Grand Library. Has no effect on Standard campaigns. |
| `narration_mode` | TEXT | `economy` \| `balanced` \| `quality` — DM-configured narration tier behaviour, default `balanced` |
| `pvp_enabled` | BOOLEAN | Default `false`. When `true`, players on the same team can trigger opportunity attacks against each other. Passed to `checkOpportunityAttacks` at every movement check. |
...
| `archetype` | TEXT | `"melee"` \| `"ranged"` \| `"caster"` \| `"support"` — used by the NPC OA decision gate; ranged and caster archetypes skip opportunity attacks. |
| `aggression` | TEXT | `"passive"` \| `"neutral"` \| `"aggressive"` — NPC OA decision gate 3; passive always skips, aggressive always takes, neutral checks HP threshold. **Null for non-combat NPCs.** |
| `reckless` | BOOLEAN | Default `false`. When `true`, this NPC always takes opportunity attacks regardless of archetype, aggression, or HP (gate 1 in the NPC OA decision chain). |
| `updated_at` | TIMESTAMPTZ | |
...
**`combat_action_type` enum (separate from `event_type` on `event_log` — named to avoid collision):**
```sql
...
Chunked lore content authored during Epic world gen. Faction histories, world history,
key location lore, named NPC backstories. Stored as chunks for semantic search.
The Orchestrator runs similarity search against this table to inject relevant lore context
into DM Specialist and NPC Specialist calls during play. All lore is available to all
specialists — the `npc-dialogue.md` skill file governs whether NPCs hint or reveal.
...
| `id` | UUID PK | |
| `campaign_id` | UUID FK → `campaigns` | |
| `title` | TEXT | Section heading — e.g. "The Fall of the Ember Throne", "Garrick's Past" |
| `content` | TEXT | Raw lore text for this chunk |
| `embedding` | vector(768) | Ollama `nomic-embed-text` embedding of `content` — generated by BYO20's bundled Ollama instance, never by the DM's LLM provider |
| `category` | TEXT | `faction_history` \| `world_history` \| `location_lore` \| `npc_backstory` \| `prophecy` |
| `source_id` | UUID | FK to the faction/npc/zone this lore belongs to — **nullable** |
| `source_type` | TEXT | `faction` \| `npc` \| `zone` — **nullable** |
| `created_at` | TIMESTAMPTZ | |
...
| `heightmap_chunk` | BYTEA | LZ4-compressed Float32Array heightmap data for this hex — null until terrain gen runs for this zone |
| `content` | JSONB | Location refs, NPC refs, world_object IDs, encounter zones, quest hooks present in this hex — populated on full gen. References world_object IDs only — never duplicates object state. |
...
#### `world_objects`
Persistent world entities that have meaningful game state beyond NPCs and containers.
Buildings, structures, landmarks, siege equipment — anything the AI DM might reference
by name or that can be interacted with, damaged, or destroyed.

Rule: if destroying or interacting with it has mechanical consequences, OR if the AI DM
would plausibly reference it by name → it gets a row. Scenery (individual trees, road
segments, generic market stalls) does not get a row — zone type and narration handle that.

| Column | Type | Notes |
|---|---|---|
| `id` | UUID PK | |
| `campaign_id` | UUID FK → `campaigns` | |
| `zone_q` | INTEGER | Axial coordinate of containing zone |
| `zone_r` | INTEGER | Axial coordinate of containing zone |
| `name` | TEXT | e.g. "The Rope Bridge", "Hermit's Cabin", "Old Mill Waterwheel" |
| `object_type` | TEXT | `building` \| `structure` \| `landmark` \| `siege_equipment` \| `vehicle` |
| `status` | TEXT | `intact` \| `damaged` \| `destroyed` \| `locked` \| `sealed` |
| `position` | JSONB | `{ x, y, z }` world position |
| `owner_type` | TEXT | `world` \| `faction` \| `npc` — **null if unowned** |
| `owner_id` | UUID | FK to owner — **null if unowned** |
| `properties` | JSONB | Object-specific mechanical state — capacity, trap state, contents, etc. |
| `created_at` | TIMESTAMPTZ | |
| `updated_at` | TIMESTAMPTZ | |
...
└── cache.*                       → synced from byo20_server
    ...
    ├── world_objects             → objects in zones the party has visited
...
No campaign-specific columns (`campaign_id`, `level`, `hp`, etc.) — those live in `game.character_campaign_state` and are synced to `cache.*`.
...
**Recovery order (strict):** Redis AOF first, Postgres checkpoint only if Redis is completely unrecoverable. Never restore both simultaneously — mixing sources permanently corrupts game state.
