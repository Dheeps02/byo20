# BYO20 — Storage Layer Spec v0.1.0
...
| `schedule` | JSONB | Time-based behavior for World Sim |
| `stat_str` | INTEGER | |
| `stat_dex` | INTEGER | |
| `stat_con` | INTEGER | |
| `stat_int` | INTEGER | |
...
#### `world_zones`
Hex grid zone registry. One row per hex. Tracks generation state and content for lazy world generation. Composite PK — coordinates are the unique identifier.

| Column | Type | Notes |
|---|---|---|
| `campaign_id` | UUID FK → `campaigns` | Part of composite PK |
| `hex_q` | INTEGER | Axial coordinate q — part of composite PK |
| `hex_r` | INTEGER | Axial coordinate r — part of composite PK |
...
**Primary Key:** `(campaign_id, hex_q, hex_r)`
...
#### `srd.ruleset_version`
Single-row metadata table identifying which ruleset version the seeded `srd.*` data was built from. Checked against the active `IRulesEngine` implementation's declared compatibility on every server startup — mismatch is a hard fail, not a warning. See game-engine.md Architecture → Ruleset Version Compatibility.
...
interface IGameStateStore {
  ...
  saveCharacterState(state: CharacterCampaignState): Promise<void>
  // ...
}
...
Both interfaces have a single Postgres implementation in v1. Future implementations can be swapped via config without touching game logic.
