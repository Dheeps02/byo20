...
pgvector extension inside the existing `byo20_server` Postgres instance. The `npc_memories`, `faction_events`, and `combat_log` tables carry a `vector(768)` embedding column. Similarity search runs as a Postgres query with cosine distance ordering. Embeddings are always generated by BYO20's bundled Ollama instance using `nomic-embed-text` (768 dimensions) — see ADR 012.

## Alternatives Considered

**ChromaDB** — A dedicated vector database. Rejected because it would be a fifth sidecar process the Electron main process needs to spawn, monitor, and clean up. BYO20 already manages four sidecars (Postgres, Redis, BYO20 Ollama, and the game server — see ADR 012). Adding a fifth increases operational complexity for self-hosters and adds another thing that can fail. The query volume doesn't justify it — NPC memory lookups are infrequent relative to a production RAG pipeline.
