...
import type { IVectorStore, NPCMemory, FactionEvent, LoreEntry } from '@byo20/shared'
...
export class PostgresVectorStore implements IVectorStore {
  ...
  async upsertMemory(memory: NPCMemory): Promise<void> {
    ...
    await this.db.insert(npc_memories).values(row)
      .onConflictDoUpdate({ target: npc_memories.id, set: rest })
  }
  ...
  async queryMemories(npcId: string, _context: string, topK: number): Promise<NPCMemory[]> {
    const rows = await this.db.select().from(npc_memories)
  ...
  async upsertFactionEvent(event: FactionEvent): Promise<void> {
    ...
    const { id: _id, ...rest } = row
    await this.db.insert(faction_events).values(row)
      .onConflictDoUpdate({ target: faction_events.id, set: rest })
  }
  ...
  async queryFactionEvents(factionId: string, _context: string, topK: number): Promise<FactionEvent[]> {
    const rows = await this.db.select().from(faction_events)
  ...
  async queryFactionEventsByEmbedding(factionId: string, queryEmbedding: number[], topK: number): Promise<FactionEvent[]> {
    const vecLiteral = toVectorLiteral(queryEmbedding)
    const rows = await this.db.select().from(faction_events)
      .where(eq(faction_events.faction_id, factionId))
  ...
  async upsertLoreEntry(entry: LoreEntry): Promise<void> {
    ...
    const { id: _id, ...rest } = row
    await this.db.insert(lore_entries).values(row)
      .onConflictDoUpdate({ target: lore_entries.id, set: rest })
  }
  ...
  async queryLoreEntries(campaignId: string, queryEmbedding: number[], topK: number): Promise<LoreEntry[]> {
    const vecLiteral = toVectorLiteral(queryEmbedding)
    const rows = await this.db.select().from(lore_entries)
