...
import { combat_log, event_log, npc_dialogue_log, party_chat_log, sessions } from "./schema/server/log";
import { faction_events, npc_memories } from "./schema/server/memory";
import {
    campaign_agenda,
    campaign_milestones,
    campaign_snapshots,
...
function characterCampaignStateToRow(state: CharacterCampaignState): typeof character_campaign_state.$inferInsert {
    return {
        id: state.id,
...
function rowToFactionReputation(row: typeof character_faction_reputation.$inferSelect): FactionReputation {
    return {
        id: row.id,
        characterCampaignStateId: row.character_campaign_state_id,
...
function factionReputationToRow(rep: FactionReputation): typeof character_faction_reputation.$inferInsert {
    return {
        id: rep.id,
 ...
 */
function rowToItem(base: typeof items.$inferSelect, statsRow: unknown): Item {
    const common = {
        id: base.id,
    ...
    switch (base.item_type) {
        case "melee": {
            ...
            return {
                ...common,
                itemType: "melee",
                stats: { damageDie: s.damage_die, damageType: s.damage_type, properties: s.properties },
            };
        }
        case "ranged": {
            ...
            return {
                ...
                itemType: "ranged",
                stats: {
                    damageDie: s.damage_die,
                    damageType: s.damage_type,
                    rangeNormal: s.range_normal,
                    rangeLong: s.range_long,
                    ammoType: s.ammo_type ?? null,
                },
            };
        }
        case "armor": {
            ...
            return {
                ...
                stats: {
                    ac: s.ac,
                    armorType: s.armor_type as ArmorType,
                    stealthDisadvantage: s.stealth_disadvantage,
                    strRequirement: s.str_requirement ?? null,
                },
            };
        }
        case "focus": {
        ...
        case "consumable": {
            ...
            return {
                ...
                stats: {
                    effect: s.effect,
                    chargesMax: s.charges_max,
                    chargesCurrent: s.charges_current ?? null,
                    recharge: s.recharge as RechargeType,
                },
            };
        }
        case "magic": {
            ...
            return {
                ...
                stats: {
                    ...
                    attuned: s.attuned,
                    bonus: s.bonus ?? null,
                    chargesMax: s.charges_max ?? null,
                    chargesCurrent: s.charges_current ?? null,
                    recharge: (s.recharge as RechargeType) ?? null,
...
export class PostgresGameStateStore implements IGameStateStore {
    ...
    async getCharacter(id: string, campaignId: string): Promise<Character> {
        const [charRows, ccsRows] = await Promise.all([
            ...
            this.db
                ...
                .where(
                    and(
                        eq(character_campaign_state.character_id, id),
                        eq(character_campaign_state.campaign_id, campaignId),
                    ),
                )
                .limit(1),
        ]);
    ...
    async saveCharacter(character: Character): Promise<void> {
        ...
        await this.db.transaction(async (tx) => {
            await tx.insert(characters).values(charRow).onConflictDoUpdate({ target: characters.id, set: charRest });
            await tx
    ...
    async saveCharacterIdentity(identity: CharacterIdentity): Promise<void> {
        ...
        const { id: _id, ...rest } = row;
        await this.db.insert(characters).values(row).onConflictDoUpdate({ target: characters.id, set: rest });
    }
    ...
    async getNPCsInZone(campaignId: string, zoneId: string): Promise<NPC[]> {
        const rows = await this.db
            .select()
            .from(npcs)
            .where(and(eq(npcs.campaign_id, campaignId), sql`${npcs.location}->>'zone_id' = ${zoneId}`));
    ...
    async saveFaction(faction: Faction): Promise<void> {
        ...
        const { id: _id, ...rest } = row;
        await this.db.insert(factions).values(row).onConflictDoUpdate({ target: factions.id, set: rest });
    }
    ...
    async saveFactionReputation(rep: FactionReputation): Promise<void> {
        ...
        const { id: _id, ...rest } = row;
        await this.db
            .insert(character_faction_reputation)
            .values(row)
    ...
    async getItems(campaignId: string, ownerId: string): Promise<Item[]> {
        ...
        const [meleeStats, rangedStats, armorSts, focusStats, consumableStats2, magicStats] = await Promise.all([
            ids("melee").length
                ? this.db
                      .select()
                      .from(melee_item_stats)
                      .where(inArray(melee_item_stats.item_id, ids("melee")))
                : Promise.resolve([]),
            ids("ranged").length
                ? this.db
                      .select()
                      .from(ranged_item_stats)
                      .where(inArray(ranged_item_stats.item_id, ids("ranged")))
                : Promise.resolve([]),
            ids("armor").length
                ? this.db
                      .select()
                      .from(armor_stats)
                      .where(inArray(armor_stats.item_id, ids("armor")))
                : Promise.resolve([]),
            ids("focus").length
                ? this.db
                      .select()
                      .from(spell_focus_stats)
                      .where(inArray(spell_focus_stats.item_id, ids("focus")))
                : Promise.resolve([]),
            ids("consumable").length
                ? this.db
                      .select()
                      .from(consumable_stats)
                      .where(inArray(consumable_stats.item_id, ids("consumable")))
                : Promise.resolve([]),
            ids("magic").length
                ? this.db
                      .select()
                      .from(magic_item_stats)
                      .where(inArray(magic_item_stats.item_id, ids("magic")))
                : Promise.resolve([]),
        ]);
    ...
    async saveItem(item: Item): Promise<void> {
        ...
        await this.db.transaction(async (tx) => {
            await tx.insert(items).values(baseRow).onConflictDoUpdate({ target: items.id, set: baseRest });

            switch (item.itemType) {
                case "melee": {
                    const s = item.stats;
                    const statsRow: typeof melee_item_stats.$inferInsert = {
                        item_id: item.id,
                        damage_die: s.damageDie,
                        damage_type: s.damageType,
                        properties: s.properties ?? undefined,
                    };
                    const { item_id: _sid, ...statsRest } = statsRow;
                    await tx
                        .insert(melee_item_stats)
                        .values(statsRow)
                        .onConflictDoUpdate({ target: melee_item_stats.item_id, set: statsRest });
                    break;
                }
                case "ranged": {
                    const s = item.stats;
                    const statsRow: typeof ranged_item_stats.$inferInsert = {
                        item_id: item.id,
                        damage_die: s.damageDie,
                        damage_type: s.damageType,
                        range_normal: s.rangeNormal,
                        range_long: s.rangeLong,
                        ammo_type: s.ammoType ?? undefined,
                    };
                    const { item_id: _sid, ...statsRest } = statsRow;
                    await tx
                        .insert(ranged_item_stats)
                        .values(statsRow)
                        .onConflictDoUpdate({ target: ranged_item_stats.item_id, set: statsRest });
                    break;
                }
                case "armor": {
                    const s = item.stats;
                    const statsRow: typeof armor_stats.$inferInsert = {
                        item_id: item.id,
                        ac: s.ac,
                        armor_type: s.armorType,
                        stealth_disadvantage: s.stealthDisadvantage,
                        str_requirement: s.strRequirement ?? undefined,
                    };
                    const { item_id: _sid, ...statsRest } = statsRow;
                    await tx
                        .insert(armor_stats)
                        .values(statsRow)
                        .onConflictDoUpdate({ target: armor_stats.item_id, set: statsRest });
                    break;
                }
                case "focus": {
                    const s = item.stats;
                    const statsRow: typeof spell_focus_stats.$inferInsert = {
                        item_id: item.id,
                        focus_type: s.focusType,
                    };
                    const { item_id: _sid, ...statsRest } = statsRow;
                    await tx
                        .insert(spell_focus_stats)
                        .values(statsRow)
                        .onConflictDoUpdate({ target: spell_focus_stats.item_id, set: statsRest });
                    break;
                }
                case "consumable": {
                    const s = item.stats;
                    const statsRow: typeof consumable_stats.$inferInsert = {
                        item_id: item.id,
                        effect: s.effect as (typeof consumable_stats.$inferInsert)["effect"],
                        charges_max: s.chargesMax,
                        charges_current: s.chargesCurrent ?? undefined,
                        recharge: s.recharge,
                    };
                    const { item_id: _sid, ...statsRest } = statsRow;
                    await tx
                        .insert(consumable_stats)
                        .values(statsRow)
                        .onConflictDoUpdate({ target: consumable_stats.item_id, set: statsRest });
                    break;
                }
                case "magic": {
                    const s = item.stats;
                    const statsRow: typeof magic_item_stats.$inferInsert = {
                        item_id: item.id,
                        custom_name: s.customName ?? undefined,
                        rarity: s.rarity,
                        attunement: s.attunement,
                        attuned: s.attuned,
                        bonus: s.bonus ?? undefined,
                        charges_max: s.chargesMax ?? undefined,
                        charges_current: s.chargesCurrent ?? undefined,
                        recharge: s.recharge ?? undefined,
                        properties: s.properties ?? undefined,
                    };
                    const { item_id: _sid, ...statsRest } = statsRow;
                    await tx
                        .insert(magic_item_stats)
                        .values(statsRow)
                        .onConflictDoUpdate({ target: magic_item_stats.item_id, set: statsRest });
                    break;
                }
            }
        });
    ...
    async transferItem(itemId: string, newOwnerId: string, newOwnerType: string): Promise<void> {
        await this.db.update(items).set({ owner_id: newOwnerId, owner_type: newOwnerType }).where(eq(items.id, itemId));
    }
    ...
    async saveQuest(quest: Quest): Promise<void> {
        ...
        const { id: _id, ...rest } = row;
        await this.db.insert(quests).values(row).onConflictDoUpdate({ target: quests.id, set: rest });
    }
    ...
    async saveWorldZone(zone: WorldZone): Promise<void> {
        ...
        const { id: _id, ...rest } = row;
        await this.db.insert(world_zones).values(row).onConflictDoUpdate({ target: world_zones.id, set: rest });
    }
    ...
    async saveWorldObject(obj: WorldObject): Promise<void> {
        ...
        const { id: _id, ...rest } = row;
        await this.db.insert(world_objects).values(row).onConflictDoUpdate({ target: world_objects.id, set: rest });
    }
    ...
    async createSession(campaignId: string): Promise<Session> {
        const rows = await this.db.insert(sessions).values({ campaign_id: campaignId }).returning();
        return rowToSession(rows[0]);
    }

    /** Set ended_at and summary on a session row. */
    async endSession(sessionId: string, summary: string): Promise<void> {
        await this.db.update(sessions).set({ ended_at: new Date(), summary }).where(eq(sessions.id, sessionId));
    }
    ...
    async createSnapshot(
        ...
        await this.db.transaction(async (tx) => {
            const existing = await tx
                .select({ id: campaign_snapshots.id })
            ...
            const ccsRows = await tx
                .select({ id: character_campaign_state.id })
                .from(character_campaign_state)
                .where(eq(character_campaign_state.campaign_id, campaignId));
            const ccsIds = ccsRows.map((r) => r.id);

            const npcRows = await tx.select({ id: npcs.id }).from(npcs).where(eq(npcs.campaign_id, campaignId));
            const npcIds = npcRows.map((r) => r.id);

            const factionRows = await tx
                .select({ id: factions.id })
                .from(factions)
                .where(eq(factions.campaign_id, campaignId));
            const factionIds = factionRows.map((r) => r.id);

            const encounterRows = await tx
                .select({ id: encounters.id })
                .from(encounters)
                .where(eq(encounters.campaign_id, campaignId));
            ...
            const [
                ...
                ccsIds.length
                    ? tx
                          .select()
                          .from(character_classes)
                          .where(inArray(character_classes.character_campaign_state_id, ccsIds))
                    : Promise.resolve([]),
                ccsIds.length
                    ? tx
                          .select()
                          .from(character_faction_reputation)
                          .where(inArray(character_faction_reputation.character_campaign_state_id, ccsIds))
                    : Promise.resolve([]),
                factionIds.length
                    ? tx
                          .select()
                          .from(faction_relationships)
                          .where(inArray(faction_relationships.faction_a_id, factionIds))
                    : Promise.resolve([]),
                ...
                encounterIds.length
                    ? tx.select().from(initiative_entries).where(inArray(initiative_entries.encounter_id, encounterIds))
                    : Promise.resolve([]),
                npcIds.length
                    ? tx.select().from(npc_memories).where(inArray(npc_memories.npc_id, npcIds))
                    : Promise.resolve([]),
                factionIds.length
                    ? tx.select().from(faction_events).where(inArray(faction_events.faction_id, factionIds))
                    : Promise.resolve([]),
    ...
    async rollbackToSnapshot(snapshotId: string): Promise<void> {
        ...
        await this.db.transaction(async (tx) => {
            ...
            if (factionIds.length)
                await tx.delete(faction_events).where(inArray(faction_events.faction_id, factionIds));
            if (encounterIds.length)
                await tx.delete(initiative_entries).where(inArray(initiative_entries.encounter_id, encounterIds));
            if (ccsIds.length)
                await tx
                    .delete(character_classes)
                    .where(inArray(character_classes.character_campaign_state_id, ccsIds));
            if (ccsIds.length)
                await tx
                    .delete(character_faction_reputation)
                    .where(inArray(character_faction_reputation.character_campaign_state_id, ccsIds));
            if (factionIds.length)
                await tx.delete(faction_relationships).where(inArray(faction_relationships.faction_a_id, factionIds));
            ...
            if (s.character_campaign_state?.length)
                await tx
                    .insert(character_campaign_state)
                    .values(s.character_campaign_state as (typeof character_campaign_state.$inferInsert)[]);
            if (s.character_classes?.length)
                await tx
                    .insert(character_classes)
                    .values(s.character_classes as (typeof character_classes.$inferInsert)[]);
            if (s.character_faction_reputation?.length)
                await tx
                    .insert(character_faction_reputation)
                    .values(s.character_faction_reputation as (typeof character_faction_reputation.$inferInsert)[]);
            if (s.faction_relationships?.length)
                await tx
                    .insert(faction_relationships)
                    .values(s.faction_relationships as (typeof faction_relationships.$inferInsert)[]);
            if (s.quests?.length) await tx.insert(quests).values(s.quests as (typeof quests.$inferInsert)[]);
            if (s.world_zones?.length)
                await tx.insert(world_zones).values(s.world_zones as (typeof world_zones.$inferInsert)[]);
            if (s.world_objects?.length)
                await tx.insert(world_objects).values(s.world_objects as (typeof world_objects.$inferInsert)[]);
            if (s.campaign_agenda?.length)
                await tx.insert(campaign_agenda).values(s.campaign_agenda as (typeof campaign_agenda.$inferInsert)[]);
            if (s.campaign_milestones?.length)
                await tx
                    .insert(campaign_milestones)
                    .values(s.campaign_milestones as (typeof campaign_milestones.$inferInsert)[]);
            if (s.narration_pool?.length)
                await tx.insert(narration_pool).values(s.narration_pool as (typeof narration_pool.$inferInsert)[]);
            if (s.encounters?.length)
                await tx.insert(encounters).values(s.encounters as (typeof encounters.$inferInsert)[]);
            if (s.initiative_entries?.length)
                await tx
                    .insert(initiative_entries)
                    .values(s.initiative_entries as (typeof initiative_entries.$inferInsert)[]);
            if (s.items?.length) await tx.insert(items).values(s.items as (typeof items.$inferInsert)[]);
            if (s.containers?.length)
                await tx.insert(containers).values(s.containers as (typeof containers.$inferInsert)[]);
            if (s.npc_memories?.length)
                await tx.insert(npc_memories).values(s.npc_memories as (typeof npc_memories.$inferInsert)[]);
            if (s.faction_events?.length)
                await tx.insert(faction_events).values(s.faction_events as (typeof faction_events.$inferInsert)[]);
        });
    }
}
