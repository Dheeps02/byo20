...
async function fetchAll(baseUrl: string, endpoint: string): Promise<unknown[]> {
    ...
    return Promise.all(list.results.map((item) => fetch(`${baseUrl}/${endpoint}/${item.index}`).then((r) => r.json())));
}
...
function transformMonster(m: Record<string, unknown>) {
    return {
        ...
        condition_immunities: ((m.condition_immunities as Array<Record<string, unknown>>) ?? []).map(
            (c) => c.index as string,
        ),
...
function transformBackground(b: Record<string, unknown>) {
    ...
    return {
        ...
        tool_profs:
            fixedToolProfs.length > 0 || choiceToolProfs.length > 0
...
async function transformClass(c: Record<string, unknown>) {
    const levels = (await fetch(`${BASE_2014}/classes/${c.index}/levels`).then((r) => r.json())) as Array<
        ...
        ? Object.fromEntries(
              levels
                  ...
                  .map((lvl) => {
                      ...
                      const slots = [1, 2, 3, 4, 5, 6, 7, 8, 9].map(
                          (n) => (sc[`spell_slots_level_${n}`] as number) ?? 0,
                      );
...
function transformItem(i: Record<string, unknown>) {
    ...
    return {
        ...
        name: i.name,
        description:
            Array.isArray(i.description) && (i.description as string[]).length > 0
...
function transformMagicItem(i: Record<string, unknown>) {
    return {
        id: i.index,
        name: i.name,
        description: (i.desc as string) ?? null,
...
async function main() {
    ...
    await Promise.all([
        write(
            "monsters.json",
            applyMonsterPatches(monsters.map((m) => transformMonster(m as Record<string, unknown>))),
        ),
        write(
            "species.json",
            species.map((s) => transformSpecies(s as Record<string, unknown>)),
        ),
        write(
            "backgrounds.json",
            backgrounds.map((b) => transformBackground(b as Record<string, unknown>)),
        ),
        write(
            "subclasses.json",
            subclasses.map((s) => transformSubclass(s as Record<string, unknown>)),
        ),
        write(
            "feats.json",
            feats.map((f) => transformFeat(f as Record<string, unknown>)),
        ),
        write(
            "items.json",
            srdItems.map((i) => transformItem(i as Record<string, unknown>)),
        ),
        write(
            "magic_items.json",
            magicItems.map((i) => transformMagicItem(i as Record<string, unknown>)),
        ),
        write(
            "conditions.json",
            conditions.map((c) => transformCondition(c as Record<string, unknown>)),
        ),
        write(
            "weapon_masteries.json",
            weaponMasteries.map((w) => transformWeaponMastery(w as Record<string, unknown>)),
        ),
    ]);
