...
export function computeSphere(
  ...
  const r = radiusFt / 5
  return candidates
    .filter(e => distance(origin, e.position) - e.boundingRadius <= r)
    .map(e => e.id)
}
...
export function computeCube(
  ...
  return candidates
    .filter(e => {
      ...
      return (
        e.position.x >= origin.x - r && e.position.x <= origin.x + s + r &&
        e.position.y >= origin.y - r && e.position.y <= origin.y + s + r &&
        e.position.z >= origin.z - r && e.position.z <= origin.z + s + r
      )
    })
    .map(e => e.id)
}
